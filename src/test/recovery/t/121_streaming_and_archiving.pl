...
# Wait until all the failed files get archived by the master
$node_master->poll_query_until('postgres',"SELECT archived_count=failed_count FROM pg_stat_archiver", 't')
...
ok( -f $node_master->archive_dir . "/$current_walfile2",
	"$current_walfile2 exists in master's archive");
...
wait_until_file_exists($node_master, $node_master->archive_dir . "/$current_walfile3",
